...
from drone_inventory.models import User, Drone, drone_schema, drones_schema, db, ma
...
@token_required
def create_drone(current_user_token): #from token required from helpers.py
...
def get_drones(current_user_token):
    ...
    drones = Drone.query.filter_by(user_token = owner).all() #get all of them
    response = drones_schema.dump(drones)
    return jsonify(response)

@api.route('/drones/<id>', methods = ['GET'])
@token_required
def get_drone(current_user_token, id): #id from @ is coming here at id
...
@api.route('/drones/<id>', methods = ['DELETE']) #we only want to remove certain values, not all of them at the same time.
@token_required
